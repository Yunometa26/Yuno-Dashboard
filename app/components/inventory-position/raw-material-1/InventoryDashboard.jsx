...
const InventoryDashboard = () => {
        ...
        const parsed = results.data
          ...
          .map((row) => ({
            ...
            inventoryTurnoverRatio:
              parseFloat(row["Inventory Turnover ratio"]) || 0,
          }));
        setData(parsed);
        setFiltered(parsed);
      },
  ...
  useEffect(() => {
    ...
    if (filters.month)
      temp = temp.filter((d) => d.date.format("MM") === filters.month);
    if (filters.day)
      temp = temp.filter((d) => d.date.format("DD") === filters.day);
  ...
  const abcAnalysisData = (() => {
    ...
    const totalItems = Object.values(abcCounts).reduce(
      (sum, count) => sum + count,
      0
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#024673] to-[#5C99E3] text-white p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#024673] to-[#5C99E3] border rounded-xl p-6">
            ...
            <PieChart>
              ...
              <Tooltip
                ...
                formatter={(value, name) => [
                  `${value} items (${
                    abcAnalysisData.find((d) => d.name === name)?.percentage
                  }%)`,
                  name,
                ]}
              />
        ...
        <p className="text-sm text-gray-200 mt-2 text-center">
          Click on any segment to view items in that ABC class
        </p>
      </div>
          ...
          <div className="bg-gradient-to-r from-[#024673] to-[#5C99E3] rounded-xl p-6 max-w-6xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-white">
                Items in ABC Class {selectedABCClass}
              </h3>
              <button
              ...
              <table className="w-full text-white">
                ...
                <tbody>
                  {getItemsForABCClass(selectedABCClass).map((item, idx) => (
                    <tr
                      key={idx}
                      className="border-b border-white border-opacity-20 hover:bg-opacity-10"
                    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-200 mt-1">
              Based on {filtered.length} records
            </p>
          </div>
                  ...
                  data={Object.entries(
                    filtered
                      ...
                      .reduce((acc, row) => {
                        ...
                        if (!acc[month])
                          acc[month] = { month, totalITR: 0, count: 0 };
          ...
          <div
            className="w-16 h-16 bg-white rounded-lg flex items-center justify-center cursor-pointer hover:bg-gray-100"
      ...
      <div className="bg-gradient-to-r from-[#024673] to-[#5C99E3] border rounded-xl p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-white">
            Total Closing Stock & MSL by Day
          </h3>
          <div className="bg-red-500 text-white px-3 py-1 rounded-lg text-sm font-medium">
            ...
            {(() => {
              ...
              return chartData.filter((d) => d.totalClosing < d.totalMSL)
                .length;
            })()}
          </div>
        </div>
